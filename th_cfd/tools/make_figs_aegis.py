...
Pure post-processing of the validated pipeline at the FER design point (F_q 2.00,
no rescale - Aegis runs AT its design peaking):
...
# ===================================================== FER design-point stack
# Same inputs that md5-match the CFD source/BCs (qpeak 12.8 kW/m, Fz 1.29, Le 1.304),
# coolant @ 283 C / 12.8 MPa, G from the CFD medium mass flux.
...
ST = dict(bulk=Tb[-1], fuel=max(r[5] for r in rows_1p),
          clad=max(r[3] for r in rows_1p), nearwall=rows_1p[pct1_i][2])
...
def f1():
    ...
    fig.suptitle("F1 · Grid convergence — ASME V&V-20 GCI  (Aegis-40 FER peak, F_q 2.00)",
...
def f4():
    ...
    a1.set_title("F4 · Hot-channel axial profiles (Aegis-40 FER peak, F_q 2.00)")
...
def f8():
    ...
    ax.annotate("FER design point\nH_tc≈4 m · G 543 · MDNBR 1.56", (4, G[1]),
