...
# ============================================================ 3-mesh GCI data
# Mesh study RUN at F_q 2.17 (field-direct, tools/meshindep.py). Single-phase +
# constant properties make T-T_in exactly linear in power, so values are rescaled
# to the NuScale-calibrated F_q 1.923 by the exact qpeak ratio (the native medium
# re-run confirms this rescale to +/-0.01 K). GCI is recomputed on the rescaled
# values; the order p and conservation ratios are power-invariant.
RESC  = 2.99e8 / 3.37e8                            # qpeak ratio  F_q 2.17 -> 1.923
_TIN  = 531.0
_rs   = lambda v: [_TIN + (x - _TIN) * RESC for x in v]
N      = [35700, 103600, 279888]                  # coarse, medium, fine
TOUT   = _rs([612.83, 613.46, 613.76])
PCT    = _rs([676.43, 676.88, 677.11])            # clad inner peak [K]
FUEL   = _rs([1181.74, 1184.24, 1186.59])
QADV   = [q * RESC for q in [25.98, 26.09, 26.15]]  # advected power [kW]
QSRC   = 26.20 * RESC                              # analytic design source [kW]
LBL    = ["coarse\n35.7k", "medium\n103.6k", "fine\n279.9k"]
...
def f1():
    ...
    fig.text(0.5, -0.015, "Mesh study run at F_q 2.17; rescaled to F_q 1.923 by the exact "
             "power ratio (single-phase linearity, verified to ±0.01 K by the native medium "
             "re-run). GCI recomputed on rescaled values.",
...
def f2():
    ...
    fig.text(0.5, -0.01, "F_q 1.923 (NuScale peak); conservation ratios are power-invariant "
             "(identical to the as-run F_q 2.17 study).",
             ha="center", fontsize=7.5, color=GREY, style="italic")
