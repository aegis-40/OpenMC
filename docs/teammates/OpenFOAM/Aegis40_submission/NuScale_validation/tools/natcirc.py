...
# -*- coding: utf-8 -*-
"""
NuScale - Natural-circulation loop solver  (keystone: fixes the core flow G)
==========================================================================

The NuScale primary loop has NO pumps: flow is driven only by the buoyancy head
...
G, settling the "G=590 vs G=1800" fork (see docs/parameters.md sec.4).
...
Defaults from docs/parameters.md; consistent with mdnbr.py / thermal_stack.py.
"""
...
@dataclass
class Config:
    # --- plant / power (docs/parameters.md sec.1,3) ---
    P_th:  float = 130.0e6     # core thermal power [W]
    p_mpa: float = 12.8        # system pressure [MPa]
    T_in:  float = 531.0       # core inlet temperature [K]

    # --- coolant transport @ T_avg ~555 K, 12.8 MPa (sec.4) ---
    rho:   float = 752.0       # density [kg/m3]
    cp:    float = 5250.0      # specific heat [J/kg.K]
    mu:    float = 9.4e-5      # dynamic viscosity [Pa.s]
    beta:  float = 2.0e-3      # volumetric thermal expansion [1/K]

    # --- core flow geometry (sec.1,2) ---
    N_pin: int   = 9768        # total fuel pins
    A_f:   float = 87.9e-6     # per-subchannel flow area [m2]
    D_h:   float = 11.77e-3    # hydraulic diameter [m]
    L_core: float = 2.0        # active (heated) length [m]

    # --- LOOP geometry: the two NuScale/MASLWR-class design knobs ---
    H_tc:   float = 6.0        # thermal-centre elevation SINK-SOURCE [m]  (riser-tall)
...
def main(argv=None):
    cfg = Config()
    ap = argparse.ArgumentParser(description="NuScale natural-circulation loop solver")
    ...
    r = solve(cfg)
    print("=" * 72)
    print(" NuScale natural-circulation loop  (1-D buoyancy balance, mdot~P^1/3)")
    print("=" * 72)
