#!/usr/bin/env pvbatch
"""
Aegis-40 - ParaView figures F9a / F9b / F9c from the PRODUCTION medium mesh (pin/, latest time).
...
Outputs (exact filenames the report embeds from docs/figs/):
  F9a_xsection_T.png - T on the sub-channel cross-section at the axial peak (z = L/2 = 1.0 m):
                       hot fuel core -> cooler coolant. Geometrically faithful (no exaggeration).
  F9b_axial_T.png    - axial T on the pin centre-plane (chopped-cosine); radial scale exaggerated
                       x26 so the thin pin (pitch 1.26 cm x L 2.0 m ~ 159:1) is legible.
  F9c_velocity.png   - coolant axial velocity U_z on the cross-section at z = 1.0 m: all-positive
...
CASE = os.path.join(ROOT, "pin")                 # production = medium mesh
FIGS = os.path.join(ROOT, "docs", "figs")
FOAM = os.path.join(CASE, "pin.foam")
ZPK = 1.0                                         # axial peak z = L/2, L = 2.0 m
EXAG = 26.0                                       # lateral exaggeration for the axial slice (F9b)
...
def main():
    ...
    # F9a: cross-section T at the axial peak (faithful, top-down)
    ...
    save(va, "F9a_xsection_T.png", 1200, 1000)
    print("  F9a T range", rA)

    # F9b: axial T centre-plane, y exaggerated x26, inlet z=0 on the left
    ...
    save(vb, "F9b_axial_T.png", 1600, 380)
    print("  F9b T range", rB)

    # F9c: coolant U_z on the cross-section at z=1.0; fuel+clad rod has no U -> grey
    ...
    save(vc, "F9c_velocity.png", 1200, 1000)
    print("  F9c U_z range", rC)

    print("DONE -> F9a_xsection_T.png  F9b_axial_T.png  F9c_velocity.png  in docs/figs/")
    print("Next: python3 tools/build_report_aegis.py")
