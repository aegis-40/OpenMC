...
# -*- coding: utf-8 -*-
"""
Aegis-40 - Correlation thermal stack  (Variant 1 post-processor)
=============================================================
...
class StackConfig:
    # radii [m]
    r_f:   float = 4.0958e-3    # fuel pellet radius   (OpenMC FUEL_RADIUS 0.40958 cm)
    r_ci:  float = 4.1873e-3    # clad inner radius    (OpenMC CLAD_INNER_R 0.41873 cm)
    r_co:  float = 4.76e-3      # clad outer radius    (OpenMC CLAD_OUTER_R 0.476 cm)
    ...
    h_gap:  float = 5246.0      # = kappaLayers/thicknessLayers = 0.48/9.15e-5 (gap 91.5 um)
    # coolant transport (for h correlation) @ 283 C / 12.8 MPa (design basis)
    mu:     float = 9.1e-5      # dynamic viscosity [Pa.s]
    k_w:    float = 0.566       # water thermal conductivity [W/m.K]
    Pr:     float = 0.87        # Prandtl
...
def run(cool, st, csv=None, table=False, boiling=False):
    ...
    print("=" * 74)
    print(" Aegis-40 correlation thermal stack  (Variant 1: CFD flow + correlation h)")
    print("=" * 74)
...
def main(argv=None):
    ...
    ap = argparse.ArgumentParser(
        description="Aegis-40 correlation thermal stack (PCT, fuel centreline)",
