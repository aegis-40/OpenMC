...
def f1():
    ...
    fig.suptitle("F1 · Grid convergence — ASME V&V-20 GCI  (Aegis-40 COLR envelope, F_q 2.4675)",
...
def f3():
    ...
    ax.set_title("F3 · CFD ↔ correlation-stack cross-check — near-wall sound (Δ < 9 K)")
...
def f4():
    ...
    top = max(Tfc_bo) + 0.10*(max(Tfc_bo) - 640.0)
    a2r.set_ylabel("fuel centreline  T  [K]", color=GREEN); a2r.set_ylim(640, top); a2r.grid(False)
    l1, la1 = a2.get_legend_handles_labels(); l2, la2 = a2r.get_legend_handles_labels()
    a2.legend(l1+l2, la1+la2, loc="upper center", bbox_to_anchor=(0.5, -0.22), ncol=4,
...
def f6():
    ...
    ax.set_title("F6 · Hot-channel DNBR — W-3 CHF + Tong F-factor (COLR envelope, G %.0f)" % cfg.G)
...
def f8():
    ...
    ax.annotate("FER design point\nH_tc≈4 m · G 542 · MDNBR 1.37", (4, G[1]),
