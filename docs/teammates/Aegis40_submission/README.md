...
- **SBLOCA & containment (low-pressure suppression containment + internal IRWST):**
  plant-specific mass-energy screening (`tools/f6_suppression.py`) — a bounding
  full-inventory blowdown into the 250 m³ IRWST gives a quasi-static containment
  pressure of 0.139 MPa (≤ 0.414 MPa design, margin ×3), ≥ 16 h to design pressure
  with zero heat-removal credit, and the IRWST gravity-injection head check sets the
  staged-ADS end-state requirement (≤ ≈0.20 MPa abs); suppression phenomenology
  supported by reference (SMART100 CPRSS/SISTA, CAREM-25, NUREG-0800 SRP 6.5.5) —
  see `docs/Aegis40_F3_F6_bounding.md`.
...
| `f6_suppression.py` | **Suppression-containment SBLOCA screening** — full-inventory blowdown into the IRWST, quasi-static containment pressure, no-pool counterfactual, gravity-injection head / ADS end-state. |
...
| `Aegis40_F3_F6_bounding.md` | SBLOCA & containment substantiation (suppression/IRWST screening + reference support). |
...
python3 tools/f6_suppression.py                 # suppression-containment SBLOCA screen
