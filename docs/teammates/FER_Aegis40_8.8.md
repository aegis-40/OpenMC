...
**Safety function.** Maintain **k_eff ≤ 0.95** in the spent-fuel pool by fixed criticality-safe rack geometry and fixed neutron absorbers (Boral or Metamic B₄C plates) with burnup credit, giving k(95/95) ≤ 0.95 (full criticality basis in §8.11) [ANSI/ANS-8.1; 10 CFR 50.68; SSG-27]. Soluble boron (≈2000 ppm) is maintained in the pool water as an independent defence-in-depth reactivity margin; this does not conflict with the soluble-boron-free reactor concept, because pool boron is credited only as pool defence-in-depth and never for reactor reactivity control. The once-through, single-batch core (37 assemblies; 2224 EFPD ≈ 6.1 EFPY; discharge burnup 29.6 GWd/tHM; §8.1.5) eliminates periodic refuelling: fuel handling is limited to the initial core load and the whole-core discharge at end of cycle, minimising handling frequency and occupational dose.

**Performance.** Pool bulk ≤ 50 °C; whole-core decay heat ≈7.95 MW at discharge, falling to ≈48 kW at 1 year and ≈9 kW at 10 years (§8.11), giving a long passive boil-off grace period; the pool is sized for one full-core offload (37 assemblies) plus reserve, and the on-site dry store for at least two discharged cores (≈12 years of operation) [SSG-15].
...
**Safety function.** DC/UPS ride-through supports the >100 h passive-cooling target (§8.5) through extended station blackout; safety side isolated from non-safety by a one-way data diode/air-gap.
