...
def build_rows():
    ...
    comparison = [
        dict(reactor="Aegis-40 (ours, once-through)", pth=125.0, pe=40.0, eta=aegis_eta,
...
def write_report(outdir: str):
    ...
    L.append("- Source: `aegis40.back_end.fuel_cycle` (validated, 15/15 tests) on the "
             "LOCKED once-through design basis (`docs/competition/design-basis-locked.md`).")
    ...
    L.append(f"_The module value ({a.hm_t_per_twhe:.2f} tHM/TWhe) uses the as-modeled "
             ...
             f"gives {x['aegis_int_id']:.2f}. The ~5% spread is the known 9.39 tHM / "
             f"~29.6 GWd/tHM self-consistency band (confirm exact fresh iHM via "
    ...
    L.append("- **Discharge burnup 29.6 GWd/tHM once-through** is the dominant lever: ~1.8x "
