...
def aoo_envelope_shape(path, G, T_in=T_IN):
    """Record-shape MDNBR at the bounding AOO corner (118 %P / 80 %G), COLR envelope."""
    z_cm, cols = load_shapes(path)
    col, f_dh = SHAPE_COL["MOC ENVELOPE"]
    rel = cols[col]
    f_z = max(rel)
    f_q = f_dh * f_z
    pm, Gc = 1.18, G * 0.80
    cfg = mdnbr.Config(p_mpa=sw.P_MPA, T_in=T_in, G=Gc, cp=sw.CP, L=sw.L,
                       D_co=sw.D_CO, D_h=sw.D_H, A_flow=sw.A_F,
                       qp_hot_peak=sw.QP_AVG * f_q * pm, Fz=f_z,
                       Le_ratio=le_ratio_from_fz(f_z), n=200)
    prof = profile_from_shape(cfg, z_cm, rel, f_dh * pm)
    w3, bow, zb = minima(cfg, prof)
    print("-" * 100)
    print(" AOO corner x REAL SHAPE — MOC ENVELOPE at 118 % power / 80 % flow (record tally):")
    print(f"   W-3* {w3:.2f} (extrapolated at this G -> trend only) / "
          f"Bowring {bow:.2f} at z {zb:.2f} m  "
          f"{'PASS' if bow >= 1.30 else 'FAIL'} (>= 1.30, in-range verdict)")
...
def main(argv=None):
    ...
    if a.shapes:
        ...
        if a.aoo:
            aoo_envelope_shape(a.shapes, a.G)
    print("-" * 100)
