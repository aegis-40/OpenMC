...
def main():
    ...
    para(doc, "The core grew to a NuScale-class 37-assembly / 9 768-pin layout, halving the "
              "per-pin power versus a conventional large PWR; even at the COLR envelope the "
    ...
    para(doc, "The correlation stack is the licensed-code industry standard and carries the CHF and "
              ...
              "transfer matches Dittus-Boelter to < 9 K.", italic=True)
    ...
    para(doc, "The V&V set is run directly at the binding COLR-envelope source (F_q 2.4675, "
              "hot pin 22.40 kW). The single-phase "
    ...
    add_table(doc, ["Metric", "coarse", "medium", "fine", "GCI_fine / direct"], [
        ["Outlet T  [K]", "617.34", "618.00", "618.32", "0.07 %"],
        ["Peak clad T  [K]", "680.86", "681.32", "681.55", "0.05 %"],
        ["Near-wall coolant  [K]", "661.66", "661.93", "662.16", "0.03 % (direct)"],
    ...
    figure(doc, "F2_energy_conservation.png",
           "Figure F2. Advected power ṁ·cp·ΔT versus the analytic design source (22.40 kW/hot-pin) "
    ...
    figure(doc, "F3_cfd_vs_stack.png",
           ...
           "operating point — agreement within 9 K on every quantity.")
    add_table(doc, ["Quantity", "CFD", "Stack (1φ DB)", "Δ"], [
        ["Coolant outlet (mixing-cup, 1φ-equivalent)", "345 °C", "345 °C", "0.4 K"],
        ["Fuel centreline (peak)", "866 °C", "867 °C", "0.5 K"],
        ["Clad inner (peak)", "408 °C", "400 °C", "8.7 K"],
        ["Near-wall / clad outer", "389 °C", "382 °C", "7.3 K"],
    ], [2.4, 1.4, 1.6, 1.2], hi={3})
    para(doc, "At the identical operating point (G 544, T_in 258 °C, F_q 2.4675, single-phase) the CFD and the "
              "Dittus-Boelter stack agree within 9 K on every quantity — bulk and fuel to < 1 K, "
              "clad and near-wall to < 9 K (CFD marginally hotter, i.e. conservative). The CFD is "
    ...
    para(doc, "At the binding COLR envelope the most-peaked channel saturates below the outlet "
              ...
              "modest 15.8 kW/m peak linear power. (The envelope CFD fields show the same channel "
              "past Tsat at the top: 345 °C single-phase-equivalent mixing-cup outlet vs "
              "Tsat 330 °C — consistent with the 7.5 % exit quality.)")
    ...
    add_table(doc, ["Metric", "Result", "Limit", "Margin", "Verdict"], [
        ...
        ["  clad 1φ (CFD x-check, envelope)", "681 K / 408 °C", "—", "—", "—"],
    ...
    para(doc, "The Jens-Lottes subcooled-boiling clamp caps the clad at 353 °C; the single-phase "
              "CFD cross-check (408 °C at the envelope source) confirms the film physics (§6). All "
    ...
    bullet(doc, "The conjugate CFD is energy-conservative (×1.00, every mesh) and mesh-independent "
                "(GCI < 0.15 %), and its near-wall heat transfer matches the correlation stack to < 9 K.")
